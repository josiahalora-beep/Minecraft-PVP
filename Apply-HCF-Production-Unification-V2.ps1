...
if ($downloadServerProperties -notmatch '(?m)^level-type=DEFAULT\s*
...
Write-Host '  - authored Stylez HCF Overworld restored from checksum-locked FreeMap.rar'
Write-Host '  - 2000-block Overworld diameter centered at 0,0 (+/-1000 playable coordinates)'
Write-Host '  - authored hills/ponds/biomes/custom trees preserved; procedural wilderness normalization disabled'
Write-Host '  - sparse clustered ground cover for clean PvP visibility without sterile superflat terrain'
Write-Host '  - 9-wide gravel HCF roads follow the authored grade; no broad road-shoulder flattening'
Write-Host '  - permanent clear noon presentation: rain/thunder rejected and time automatically restored'
Write-Host '  - base scouting samples the real authored terrain and claims stay fully inside the 2k border'
...
Write-Host ''
Write-Host 'Phase-1 authored map note:' -ForegroundColor Yellow
Write-Host '  - FreeMap.rar is downloaded automatically when missing and verified by size + SHA-256.'
Write-Host '  - A fresh /sotw reset confirm + normal start restores the approved authored Overworld before structures are applied.'
Write-Host '  - The fallback procedural terrain code is not the production wilderness generator.'
Write-Host '  - Phase 2 will replace the current faction-base visual system using the frozen reference library.'
 -or
...
if ($downloadReset -notmatch 'FreeMap\.rar' -or
    $downloadReset -notmatch 'af9c214979fcde0b1c41e435a6359940a930ffa97c8e2ad09667f74203afba95') {
    throw 'Downloaded season reset is missing the verified authored HCF world restore contract.'
}
if ($downloadMapAi -notmatch 'border:\s*1000' -or
    $downloadMapAi -notmatch 'x:\s*800,\s*y:\s*64,\s*z:\s*-800' -or
    $downloadMapAi -notmatch 'z:\s*775,\s*radius:\s*175') {
    throw 'Downloaded Mineflayer map intelligence is not aligned to the authored 2k layout.'
}
if ($downloadBaseBuilder -notmatch 'terrainUphillBias' -or
    $downloadBaseBuilder -notmatch 'v14: terrain cover follows the site''s REAL uphill side' -or
    $downloadBaseBuilder -notmatch 'qa-showcase' -or
