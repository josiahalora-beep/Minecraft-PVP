...
public final class EraCore extends JavaPlugin implements Listener, CommandExecutor {
    ...
    private boolean cmdBaseRebuild(Player p,String[] a) {
        ...
        if(a.length==1 && "palettes".equalsIgnoreCase(a[0])) {
            if(!getConfig().getBoolean("base-builder.qa-showcase",false)) {
                p.sendMessage(color("&cQA showcase is disabled on this server."));
                return true;
            }
            hcfBaseBuilder.queueQaPaletteShowcase();
            p.sendMessage(color("&aQueued HCF exterior palette visual QA showcase."));
            return true;
        }
    ...
    synchronized int factionMemberCountAuthority(String factionName) {
        Faction f=factions.get(factionName==null?"":factionName.toLowerCase(Locale.ENGLISH));
        return f==null?-1:f.members.size();
    }
