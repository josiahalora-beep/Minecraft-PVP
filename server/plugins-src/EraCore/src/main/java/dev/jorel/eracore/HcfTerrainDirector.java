...
final class HcfTerrainDirector implements Listener {
    ...
    void start() {
        if(authoredWorld()) {
            plugin.getLogger().info("[terrain] authored HCF world preservation active; wilderness terrain and decoration are left untouched.");
        }
    }
    ...
    boolean queueProductionTerrain() {
        if(authoredWorld()) {
            ...
            plugin.saveConfig();
            plugin.getLogger().info("[terrain] authored HCF world is authoritative; no wilderness blocks are rewritten.");
            return true;
        }
